---
pubDate: 2026-02-06
title: Design resources
description: "Design resources that make me a finer designer."
published: yes
image:
  url: "../images/posts/design_resources_joacim_eldre.webp"
  alt: "A triangle, square, and circle on a gray background, the triangle is black, the square has the bottom traced with a mouse cursor on the end. Made by Joacim Eldre."
tags: ["All", "Resource"]
---

import TableOfContents from "@/components/global/TableOfContents.astro";

<TableOfContents />

# Fun, Inspiration

[The future of creativity on the internet in a world with AI](https://www.youtube.com/watch?v=17_HcR95YBc)

[Kode Dot](https://kode.diy/) is a tiny programmable pocket device with a touch screen.

[Color Match](https://colormatch.pages.dev/) is a browser game where you match colors on time.

[Bezier](https://bezier.method.ac/) is a browser game that helps you learn the vector pen tool.

[Pixel Art Park](https://pixelartpark.com/en/) is a Japanese event for pixel artists to share their work, and a place to find inspiration.

[Not Boring Software](https://www.notboring.software/) build fun apps for boring routines.

[Drams](https://drams.framer.website/) is a collection of Framer components inspired by Dieter Rams' design principles.

[Conversations on Quality](https://linear.app/quality) is a video series of interviews with designers, hosted by Linear.

[Design Notes](https://designnotes.fm/) is a podcast about creative work.

[User experience in LEGO interfaces](https://interactionmagic.com/UX-LEGO-Interfaces/) explores the various interfaces that exist in LEGO.

[Ollie Hoff](https://www.olliehoff.co.uk/) is an illustrator and graphic designer.

[Headspace](https://www.headspace.com/) is an app for meditation and sleep.

[Teenage Engineering](https://teenage.engineering/) make hardware products for audio engineering.

[Good Enough](https://goodenough.us/) is a company who make software that is good enough.

[The Hardest Working Font In Manhattan](https://aresluna.org/the-hardest-working-font-in-manhattan/) is a deep dive article about a font called Gorton.

[Shift Happens](https://shifthappens.site/) is a book about typewriters and keyboards.

[Morten Kantsø](http://kantsoe.com/) is an illustrator.

[Simple Bits](https://simplebits.shop/) is a tiny type foundry, goods shop, and design studio who make handmade letters and numbers.

[IORAMA](https://www.iorama.studio/) makes apps inspired by musical instruments and video games.

[A Playful Path](https://www.aplayfulpath.com/the-book/) is a book about play, being playful, and playfulness.

[Playables](https://playables.net/) create playful strange things, like video games.

# Hardware

~~Pen. My favourite pens are the Copic Multiliner SP 0.5, LAMY Safari (yellow, of course), and Pilot V7 0.5.~~

~~Paper. Anything goes, but I prefer thick and grainy paper.~~

I use a [Remarkable tablet](https://remarkable.com/) for most notetaking and sketches these days.

Tablet & stylus with pressure sensitive touch display. I use an iPad and Apple Pencil.

Computer. I use a MacBook Pro.

# Software, Apps, Web apps

[Kala Paint](https://kalapaint.app/) is a GPU based painting app with no subscription and weird stuff. Made by an artist, for artists.

[Paper](https://paper.design/) is a collaborative web and desktop application for designing and prototyping user interfaces.

[Zed](https://zed.dev/) is a modern, minimal, no bullshit open-source code editor available for free on MacOS, Linux, and Windows.

[Ghostty](https://ghostty.org/) is a tab based terminal emulator for MacOS and Linux.

[Figma](https://www.figma.com/) is a collaborative web application for wireframe and prototype design.

[Frame0](https://frame0.app/) is a MacOS, Windows, and Linux software to design wireframes.

[Affinity Studio](https://www.affinity.studio/) is a MacOS and Windows software suite for raster and vector graphics.

[Squoosh](https://squoosh.app/) is a website for optimizing images.

[Spline](https://spline.design/) is a collaborative web application for 3D sculpting, modeling, and animation.

[Cavalry](https://cavalry.app/) is a Windows and MacOS software for 2D animation. Now included in Affinity Studio.

[Rive](https://rive.app/) is a collaborative web application for 2D animation.

[Procreate](https://procreate.com/) is an iPad app for raster graphics.

[Nomad Sculpt](https://nomadsculpt.com/) is an iPad app for 3D sculpting and modeling.

[Raycast](https://www.raycast.com/) is a launcher application for MacOS and Windows.

[CleanShot X](https://cleanshot.com/) is a screen capture application for MacOS.

[Tabular](https://tabular.email/) is a web application to build responsive emails.

[Pixelorama](https://pixelorama.org) is a free and open source application for pixel art for Android, Windows, Linux, and MacOS.

[Asesprite](https://www.aseprite.org/) is a pixel art application for MacOS, Windows, and Linux.

[Pixquare](https://pixquare.art/) is an iPad and iPhone app for pixel art.

# Learn

[Visual Design](https://anthonyhobday.com/books/visualdesign) is website book about how to improve your visual design.

[Learning Through Play](https://learningthroughplay.com/) is a website with articles and guidelines for the importance of play, and being playful as a way to learn.

[Smart Interface Design Patterns](https://smart-interface-design-patterns.com/) is a course with guidelines and best practices for common components in user interfaces.

# Accessibility

[a11y, Accessibility Project](https://www.a11yproject.com/) is a website with resources and guidelines for web accessibility.

[The Micropedia](https://www.themicropedia.org/) is a collection of micro aggressions. Stuff you shouldn't do, really.

[Empathy Prompts](http://empathyprompts.net/) is a collection of small prompts that you can use to test accessibility and empathy in your designs.

# Design System

[Astryx](https://astryx.atmeta.com/) is an "agentic first" open source design system for building user interfaces.

[Atomic Design](https://atomicdesign.bradfrost.com/) is a free web book on how to build reusable parts, from small to large.

[Design Systems Database](https://designsystems.surf/) is a directory of design systems.

[Design Systems](https://www.designsystems.com/) is a design systems publication by Figma.

[Storybook](https://storybook.js.org/) is a web tool to present user interface components for development, testing, and documentation.

# Design Thinking

[Untools](https://untools.co/) is a website to learn methods for practical problem solution.

[The Design Sprint](https://www.thesprintbook.com/the-design-sprint) is a book on how to build and test a prototype in five days.

[Mindful Design](https://mindfuldesign.xyz/) is the responsible designer’s survival guide.

# Product Thinking

[Agile Manifesto](https://agilemanifesto.org/) is how you should develop software.

[When Coffee and Kale Compete](https://www.revealed.market/when-coffee-and-kale-compete) is a free PDF book on how to find and solve the core problem.

[Shape Up](https://basecamp.com/shapeup) is a free web book that makes you stop running in circles to ship work that matters.

# User Interface

[Practical UI](https://practical-ui.com/) is a book about making user interfaces that work.

[Refactoring UI](https://www.refactoringui.com/) is a book about front-end design.

[Blocks](https://www.blocks.pm/) is a low fidelity wireframe design kit for Figma.

[Layers](https://layers.to/) is a website with design inspiration for apps and websites.

[Complementary](https://podcasts.apple.com/us/podcast/complementary/id1671376839) is a podcast about interface design.

[Game User Interface Database](https://www.gameuidatabase.com/) is a website with a lot of examples of user interfaces from video games.

# User Experience

[Laws of UX](http://lawsofux.com/) is a book about the principles of user experience design.

[Built for Mars](https://builtformars.com/) is a website about user experience and digital product design.

[Apple & Banana](https://www.appleandbanana.org/) is a website filled with resources on user experience and product design.

[Growth Design Psychology](https://growth.design/) is a website about the psychology behind user experience in digital products.

[UX Signals](https://www.uxsignals.com/) is a web application to test user experience signals in your digital product.

[Raut](https://raut.no/) is a Norwegian newsletter about user experience.

[Deceptive Patterns](https://www.deceptive.design/) is a website that shame intentionally bad design patterns that manipulate the user in a bad way.

# Test, Research, Analyse

[Rocket Surgery Made Easy](https://sensible.com/rocket-surgery-made-easy/) is a book about testing what you build with your users, to verify that what you make is usable for your users. Explained in a simple and practical way that you can start doing right now.

[Don't Make Me Think](https://sensible.com/dont-make-me-think/) is a book about building usable services and products on a digital screen.

[Posthog](https://posthog.com/) is analytics for your digital product. Kind of the same as Google Analytics, but (maybe) better.

[Dovetail](https://dovetail.com/) is analytics for catching and improving user signals, also known as feedback or user pains. Same as Posthog, kind of the same as Google Analytics, but (maybe) better.

[Tally](https://tally.so/) is a web tool for building forms to collect feedback from users.

[A/B Test Design](https://abtest.design/) is a website with A/B test results from apps and websites.

# Communicate, Plan, Document, Note

[FigJam](https://www.figma.com/figjam/) is a collaborative online whiteboard for teams to brainstorm, plan, and document ideas.

[Slack](https://slack.com/) is a messaging platform for teams to communicate and collaborate.

[Obsidian](https://obsidian.md/) is a knowledge management and note-taking application that uses a local folder of Markdown files.

[Linear](https://linear.app/) is a project management tool for software development teams.

# Type, Typography, Font

[Free Faces](https://www.freefaces.gallery/) is a collection of free, open source fonts.

[Google Fonts](https://fonts.google.com/) is a collection of free, open source fonts. Curated by Google.

[Legible Typography](https://legible-typography.com/) is a book about writing readable text.

[Better Web Type](https://betterwebtype.com/) is a ton of words on how to build better digital products on the web with typography, what makes characters (letters) look like characters.

[Flexible Typesetting](https://flexibletypesetting.com/) goes deep into the world of typography. How you set text, and why setting text is important and lead to better products. Most of the stuff we humans interact with, is based on text, letters, numbers, symbols. So, why shouldn't we make that look good? Indeed.

# Color

[Color and Contrast](http://colorandcontrast.com/) is an interactive guide to color & contrast.

[Poline](https://meodai.github.io/poline/) is an enigmatic color palette generation library, that harnesses the mystical witchcraft of polar coordinates.

[Color Palette Pro](https://colorpalette.pro/) is a really fancy color palette generator based on OKLCH color theory.

[Inclusive Colors](https://www.inclusivecolors.com/) is a website that uses HSLuv to explore WCAG compatible accessible color combinations.

# Icon, Logo, Illustration

[The Noun Project](http://thenounproject.com/) is a collection of free icons and stock photos made by humans.

[Google Material Icons and Symbols](https://fonts.google.com/icons) is Google's official collections of icons.

[Streamline Icon University](https://blog.streamlinehq.com/tag/icons/) is a collection of icon design guidelines from a huge icon collective. Kind of like [FontAwesome](https://fontawesome.com/).

[Derek Yu: Pixel Art Basics Tutorial](https://www.derekyu.com/makegames/pixelart.html) is a great page with solid advice on tools and process on how to make pixel art with depth for use in video games. Derek also has a guide with [common mistakes when making pixel art](https://www.derekyu.com/makegames/pixelart2.html).

# Code

[Invent with Python](https://inventwithpython.com/) is a series of books written by Al Sweigart on Python. They're available for free to read online, and they're written in a practical way to code along with.

# Game Development

[Levelling the Playing Field](https://courses.ramiismail.com/) is a course by Rami Ismail on everything in game development that isn't game development. Business, marketing, community, and more.
